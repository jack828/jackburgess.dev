...
lastmod: '2022-06-11'
summary: 'Like many others, I couldn’t get the dashboard metrics in TrueNAS 12 working. So, I decided I would get it plugged into Grafana, for that sweet sweet data.'
---
...
We’ll need somewhere for all our work to live. Instead of cluttering up the system with many jails for each service, I like to group logically similar processes into one jail for ease of use.
...
```shell
truenas% sudo iocage create -n grafana \
  -r 12.2-RELEASE boot=on vnet=on allow_tun=1 \
  ip4_addr="vnet0|192.168.0.100/24" \
  allow_sysvipc=1 bpf=yes
```
...
Now, let’s set up our metrics processes. You’ll need to install the `influxdb` and `grafana7`[^2] packages.
...
![Correct settings for Reporting](/static/images/blogs--truenas-grafana-influxdb-metrics--truenas-system-reporting.png)
...
Head to `http://jail.ip.address:3000/datasources` and let’s set up Grafana.
Click `Add data source` and choose `InfluxDB`.

Set the URL to `http://localhost:8086` and your database to `graphite` (as we set up earlier).

![InfluxDB setup](/static/images/blogs--truenas-grafana-influxdb-metrics--influx-setup.png)

![InfluxDB database](/static/images/blogs--truenas-grafana-influxdb-metrics--influx-database.png)

Smash `Save & Test` and you should see a beautiful green banner letting you know all is good.

![InfluxDB working](/static/images/blogs--truenas-grafana-influxdb-metrics--influx-working.png)

# Templates

Now that this is done - if you’re happy setting up multiple queries on long measurements then we’re all done! Head to Grafana and set up the dashboard as you like!
...
This isn’t necessarily a _bad_ way of storing the metric - but you’ll need many queries equal to the number of disks you have to show this in Grafana! If you ever add more disks, you’ll need to amend all your graphs to have the additional query.
...
![Grafana default query](/static/images/blogs--truenas-grafana-influxdb-metrics--grafana-default-query.png)
...
![Grafana disktemp graph](/static/images/blogs--truenas-grafana-influxdb-metrics--grafana-disktemp-graph.png)
...
![Grafana disktemp final result](/static/images/blogs--truenas-grafana-influxdb-metrics--grafana-disktemp-final.png)
...
Thanks for reading and I hope you’ve successfully set up a beautiful metrics dashboard!
