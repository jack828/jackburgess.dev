...
nestedPost (optional, boolean, if true, hides post from lists etc. Only viewable from within a nested post parent.)
order (optional, number, if set, will dictate which order posts with the same date are in. Useful for nested posts.)
disableComments (optional, boolean, if true, will disable commenting on this post.)
```
